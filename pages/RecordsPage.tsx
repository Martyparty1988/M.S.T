...
const RecordsPage: React.FC = () => {
  ...
  const getProjectName = (id: string | null) => id ? (projects.find(p => p.id === id)?.name || t('records_unknown_project')) : '';
  
  const sortedEntries = useMemo(() => {
    const filteredEntries = selectedWorkerFilterId
      ? workEntries.filter(entry => entry.workerIds.includes(selectedWorkerFilterId))
      : workEntries;
    return [...filteredEntries].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  }, [workEntries, selectedWorkerFilterId]);
    ...
    switch (entry.type) {
        ...
        case 'task':
            switch (entry.subType) {
                ...
                case 'cables':
                    return <p className="text-sm text-white/70">{t('records_entry_type_cables')}: <span className="font-medium text-white">{t('records_table_label')} {entry.table} ({entry.tableSize})</span></p>;
            }
    }
  }
  ...
  return (
    <div className="h-full w-full flex flex-col p-4 overflow-y-auto">
      <div className="mb-4">
          <label htmlFor="workerFilter" className={formLabelStyle}>{t('records_filter_by_worker')}</label>
          <select
              id="workerFilter"
      ...
      <div className="glassmorphism p-4 rounded-xl flex-grow border border-white/20 shadow-lg">
        <h2 className="text-xl font-bold mb-3 text-white">{t('records_all_entries_title')}</h2>
        <div className="space-y-3">
          {sortedEntries.map(entry => {
            return (
              <div key={entry.id} className="bg-white/5 p-3 rounded-lg flex justify-between items-center transition hover:bg-white/10">
